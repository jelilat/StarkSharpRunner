...
using static AdronCore.Fusion.Sharpion.Dotnet.Handlers.Enum;
using static AdronCore.Fusion.Sharpion.Dotnet.Handlers.Packs;

namespace AdronCore.Fusion.Sharpion.Dotnet.Handlers
{
    public class Handler
    {
        ...
        public static async Task HandShake(string datahandjson)
        {
            try
            {
                ...
                Packs.Packet handshakepacket = JsonConvert.DeserializeObject<Packs.Packet>(datahandjson);

                switch (handshakepacket.type)
                {
                    case (int)Enum.ClientEnum.Login:
                        await HandleLoginPacketAsync(JsonConvert.DeserializeObject<Packs.LoginPacket>(datahandjson));
                        break;
                    case (int)Enum.ClientEnum.WalletPack:
                        await HandleConnectionPacketAsync(JsonConvert.DeserializeObject<Packs.ConnectionWalletPack>(datahandjson));
                        break;
                    case (int)Enum.ClientEnum.Balance:
                        await HandleBalancePacketAsync(JsonConvert.DeserializeObject<Packs.BalancePacket>(datahandjson));
                        break;
                    case (int)Enum.ClientEnum.Transaction:
                        await HandleTransactionPacketAsync(JsonConvert.DeserializeObject<Packs.TransactionPacket>(datahandjson));
                        break;
                    default:
                        break;
                }
        ...
        public static async Task HandleLoginPacketAsync(Packs.LoginPacket loginPacket)
        {
        ...
        public static async Task HandleConnectionPacketAsync(Packs.ConnectionWalletPack connectionPacket)
        {
        ...
        public static async Task HandleBalancePacketAsync(Packs.BalancePacket balancePacket)
        {
            // Safeguard against null packets to prevent potential NullReferenceException.
        ...
        public static async Task HandleTransactionPacketAsync(Packs.TransactionPacket transactionPacket)
        {
            // Safeguard against null packets to prevent potential NullReferenceException.
